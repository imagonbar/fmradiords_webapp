...
function geoPos(){
	if (navigator.geolocation) {
		...
		//navigator.geolocation.watchPosition(getLatituteLongitude, errorFunction);
	} else {
		console.log("Errorr");
...
function getLatituteLongitude(pos) {
    ...
    localStorage.setItem('mylatit', mylat);
   	//document.getElementById('hiddenInfo_latitude').value = mylat; 

    var mylong = pos.coords.longitude;
    localStorage.setItem('mylongit', mylong);
   	//document.getElementById('hiddenInfo_longitude').value = mylong;  
}
...
function openDB(){
	var request = window.indexedDB.open("BD_radios_prueba_3", 1);
	request.onsuccess = function(event) {
		console.log("Success opening DB - Radios");
	...
	request.onupgradeneeded = function(event) {
		...
		objectStore.createIndex('nombre_rfe', 'nombre_rfe', { unique: false });
		objectStore.createIndex('frecuencia_rfe', 'frecuencia_rfe', { unique: false });
		//objectStore.createIndex('nombre_frecuencia_rfe', ['nombre_rfe','frecuencia_rfe'], { unique: false });
...
function initDB(){
	var objectStore = db.transaction("table_radios").objectStore("table_radios");
}

function initDB_nombre_rfe(){
	var objectStore = db.transaction("table_radios").objectStore("table_radios");
	return objectStore.index('nombre_rfe');
}

function initDB_frecuencia_rfe(){
	var objectStore = db.transaction("table_radios").objectStore("table_radios");
	return objectStore.index("frecuencia_rfe");
}

/*function initDB_nombre_frecuencia_rfe(){
	var objectStore = db.transaction("table_radios").objectStore("table_radios");
	return objectStore.index("nombre_rfe");
}*/

function initDB_latitud(){
	var objectStore = db.transaction("table_radios").objectStore("table_radios");
	return objectStore.index("latitud");
}

function initDB_longitud(){
	var objectStore = db.transaction("table_radios").objectStore("table_radios");
	return objectStore.index("longitud");
}
